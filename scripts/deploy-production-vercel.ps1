...
$deploymentRoot = Join-Path $temporaryBase ("student-assessment-production-" + [guid]::NewGuid().ToString("N"))
...
try {
  ...
  if ($LASTEXITCODE -ge 8) {
    throw "Could not stage the production deployment. Robocopy exit code: $LASTEXITCODE"
  }

  & $vercelCli link `
    --yes `
    --project student-assessment-app `
    --scope stevemackidd-7217s-projects `
    --cwd $deploymentRoot `
    --no-color

  if ($LASTEXITCODE -ne 0) {
    throw "Could not link the staged source to the Vercel production project."
  }
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "The Vercel production deployment failed."
  }
}
finally {
